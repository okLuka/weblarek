...
// Утилита
function ensureElement<T extends Element>(selector: string, parent: ParentNode = document): T {
...
function bootstrap() {
  ...
  events.on('buyer:updated', () => {
    ...
    if (paymentForm) {
      ...
      paymentForm.setErrors({ payment: v.errors.payment, address: v.errors.address });
      paymentForm.setSubmitDisabled(Boolean(v.errors.payment || v.errors.address));
    }
    if (contactsForm) {
  ...
  events.on('form:submit', () => {
    ...
    if (contactsForm) {
      if (!v.errors.email && !v.errors.phone) {
        ...
        server.createOrder({ payment, address, email, phone, total, items })
          .then((res) => {
