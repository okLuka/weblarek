...
export abstract class BaseFormView {
  ...
  constructor(template: HTMLTemplateElement, events: IEvents) {
    this.events = events;
    this.el = (template.content.firstElementChild as HTMLFormElement).cloneNode(true) as HTMLFormElement;
    this.submitBtn = this.el.querySelector('button[type="submit"], .button[type="submit"], .form__submit');
    ...
    this.el.addEventListener("input", () => this.events.emit("form:change", this.collectValues()));
    this.el.addEventListener("change", () => this.events.emit("form:change", this.collectValues()));
  }
  ...
  setSubmitDisabled(disabled: boolean): void { if (this.submitBtn) this.submitBtn.disabled = disabled; }
  ...
  clearErrors(): void { if (this.errorsBox) this.errorsBox.textContent = ""; }
}